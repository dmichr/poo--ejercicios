...
class Archivo:
    ...
    def escribirL(self, datos, modo):
        with open(self.__archivo, modo, encoding="UTF-8") as file:
           for dato in datos:
             file.write(dato)
    
    def escribirWSD(self, datos, modo):
        with open(self.__archivo, modo, encoding="UTF-8") as file:
           for dato in datos:
             file.write('\n'+dato)
    ...
    def eliminar(archivop, archivos, codigo):
      with open(archivop, "r") as input:
        with open(archivos, "w") as output:
            # iterate all lines from file
            for line in input:
                # if line starts with substring 'time' then don't write it in temp file
              if not line.strip("\n").startswith(codigo):
                output.write(line)
      os.replace(archivos, archivop)  
    
    def leerArchivo(self):
      with open (self.__archivo ,mode='r',encoding='utf-8') as fp:
        lines = fp.readlines()
        r = []
        for line in lines:
            r.append(str(line))
